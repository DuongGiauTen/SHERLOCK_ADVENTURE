...
Position RobotC::getNextPosition() {
	Position criminalNextPos = criminal->getCurrentPosition();
	...
	bool uniqueOptimal = true; // Giả sử ban đầu vị trí tối ưu là duy nhất
	...
	for (const Position& dir : directions) {
		...
		if (map->isValid(potentialPos, this)) {
			int distance = pointDistance(potentialPos, criminalNextPos);

			if (distance < minDistance) {
				minDistance = distance;
				bestPos = potentialPos;
				uniqueOptimal = true; // Tìm thấy vị trí tối ưu mới, đặt lại uniqueOptimal
			}
			else if (distance == minDistance && !potentialPos.isEqual(criminalNextPos)) {
				// Có nhiều vị trí tối ưu, đánh dấu uniqueOptimal là false
				uniqueOptimal = false;
			}
		}
	}

	// Nếu vị trí tối ưu là duy nhất, cho phép di chuyển vào vị trí của Criminal
	if (uniqueOptimal) {
		return bestPos;
	}
	else {
		// Nếu có nhiều vị trí tối ưu, tránh di chuyển vào vị trí của Criminal
		for (const Position& dir : directions) {
			Position potentialPos(pos.getRow() + dir.getRow(), pos.getCol() + dir.getCol());

			if (map->isValid(potentialPos, this) &&
				!potentialPos.isEqual(criminalNextPos) &&
				pointDistance(potentialPos, criminalNextPos) == minDistance) {
				return potentialPos;
			}
		}
	}
...
void RobotC::move() {
	// Lưu trữ vị trí hiện tại của Criminal
	Position criminalCurrentPos = oldPos;

	// Luôn cố gắng di chuyển đến vị trí hiện tại của Criminal
	if (map->isValid(criminalCurrentPos, this)) {
		pos = criminalCurrentPos;
		nextPosition = criminalCurrentPos; // Cập nhật nextPosition cho lượt sau
	}
	else {
		// Nếu vị trí của Criminal không hợp lệ, di chuyển theo chiều kim đồng hồ
		Position nextPos = getNextPosition();
		if (nextPos.getCol() != Position::npos.getCol()) {
			pos = nextPos;
		}
		nextPosition = oldPos; // Cập nhật nextPosition cho lượt sau
	}
}
